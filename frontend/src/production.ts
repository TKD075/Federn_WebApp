...
function renderProjects(host: HTMLElement, projects: Project[]): void {
...
async function loadProjects(): Promise<void> {
  ...
  try {
    ...
    if (!Array.isArray(data) || data.length === 0) {
      host.innerHTML = '<p class="lead">まだデータがありません。</p>';
      return;
    }
    renderProjects(host, data);
  } catch (err) {
    host.innerHTML = `<p class="lead">バックエンドに接続できません: ${escapeHTML(String(err))}</p>`;
  }
}
